...
class OodMixupTrainer:
    def __init__(self, net: nn.Module, train_loader: DataLoader,
        ...
        self.optimizer = torch.optim.SGD(
            net.model.prompt_learner.parameters(),
            config.optimizer.lr,
...
class OodMixupTrainer_idood:
    def __init__(self, net: nn.Module, train_loader: DataLoader,
        ...
        if config.optimizer.name == 'sgd':
            ...
            # pdb.set_trace()
            self.optimizer = torch.optim.Adam(net.model.prompt_learner.parameters(), lr=config.optimizer.lr, betas=(0.9, 0.999), eps=1e-06, weight_decay=config.optimizer.weight_decay)
        elif config.optimizer.name == 'adamw':
            ## eps=1e-8 lead to nan.
            self.optimizer = torch.optim.AdamW(net.model.prompt_learner.parameters(), lr=config.optimizer.lr, betas=(0.9, 0.999), eps=1e-06, weight_decay=config.optimizer.weight_decay)
        else:
            raise NotImplementedError
