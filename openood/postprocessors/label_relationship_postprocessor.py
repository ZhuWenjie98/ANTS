...
class LabelRelationPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        ...
        # 定义文本文件的路径
        files = {
        './scorefiles/id_mcm_score_gt_list.txt': id_mcm_score_gt_list,
        './scorefiles/id_neglabel_score_gt_list.txt': id_neglabel_score_gt_list,
        './scorefiles/id_mcm_score_list.txt': id_mcm_score_list,
        './scorefiles/id_neglabel_score_list.txt': id_neglabel_score_list,
        }
        ...
        #检查文件是否存在
        if all(os.path.exists(file_name) for file_name in files.keys()):  # 文件存在
            for file_name, data_list in files.items():
                   ...
                   for i, line in enumerate(file):
                    if line == '[]\n':
                       continue
                        ...
                        # 将每行的字符串转换为浮点数，并创建张量
                        sublist = [torch.tensor([float(num)]) for num in line.strip().split()]
                        data_list[i] = sublist
            ...
            for batch in tqdm(id_loader, disable=False):
                ...
                #查看velvet类dclip logit
                # for i in range(len(label)):
                #     if label[i] == 885:
                #         dclip_output = logit_scale * image_features[i] @ text_features_dclip.t() # batch * class.
                #         id_dclip_logits.append(dclip_output)
                ...
                if not os.path.exists(file_name):
                    with open(file_name, 'w') as file:
                        for sublist in data_list:
                            if sublist:  # 检查是否为空列表   
                                file.write(' '.join(str(item.item()) for item in sublist)+ '\n')  # 使用 item
                            else:
                                file.write('[]\n')  # 写入空列表的表示    
        ...
        for batch in tqdm(ood_loader,
            ...
            #     if pred_in[i] == 885:
            #         dclip_output = logit_scale * image_features[i] @ text_features_dclip.t() # batch * class.
            #         ood_dclip_logits.append(dclip_output)
            ############################### only score in. here is a 
            output_only_in = output[:, :class_num]
            score_only_in = torch.softmax(output_only_in / self.tau, dim=1)
            mcm_score, pred_only_in = torch.max(score_only_in, dim=1)
        ...
        for i in range(class_num):
            ...
            if len(ood_neglabel_score_list[i]) != 0:
                count = sum(1 for score in ood_neglabel_score_list[i] if score > 0.9)
                ood_error_count.append(count)
        ...
        # 使用列表推导
        total_count = [id + ood for id, ood in zip(id_prection_count, ood_prection_count)]
        indices = [index for index, value in enumerate(total_count) if value > 100]
        pdb.set_trace()
