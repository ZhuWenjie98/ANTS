...
path="./txtfiles"
save_path="./txtfiles/wordnet_imagenet_cossim_dedup.pth"

# adj_imagenet_template = ['This is a {} photo.']
# noun_template = ['The nice {}.']
...
# ipdb.set_trace()
######################################## 这里去重做的不彻底，可能是因为大小写或者特殊字符等原因，反正目前noun & adj 里存在和 imagenet classname 相同的类别！！！
...
# train_cls = sorted(glob.glob('{}/*'.format(img_path)))
# shot=36
...
#     selected_list.append(selected_ims)
#     # pdb.set_trace()
...
# prename = '/home/notebook/code/personal/S9052995/syn_image/' ## pre_image name
# len_pre = len(prename)
# with open(save_path,'w') as f:
#     for name in category:
#         label=category.index(name)
#         files = selected_list[label]
#         # sub_path=path+'/'+name
#         # files=os.listdir(sub_path)
#         for file in files:
#             # line=prefix+name+'/'+file+' '+str(label)+'\n'
#             line = file[len_pre:] +' '+str(label)+'\n'
#             f.write(line)
#     f.close()       
