...
from .clip_fixed_llm_ood_prompt import FixedCLIP_LlmOODPrompt
...
def get_network(network_config):
    ...
    elif network_config.name == 'fixedclip_llmoodprompt':
        net = FixedCLIP_LlmOODPrompt(network_config)
    ...
    if network_config.pretrained:
        if type(net) is dict:
            ...
            try:
                ...
                loaded_pth.pop('model.prompt_learner.token_prefix')
                loaded_pth.pop('model.prompt_learner.token_suffix')
                loaded_pth.pop('model.prompt_learner.ood_token_prefix')
                loaded_pth.pop('model.prompt_learner.ood_token_suffix')

                # loaded_state_dict = loaded_pth['state_dict']
                # for key in current_state_dict.keys():
                #     if key in loaded_state_dict:
                #         current_state_dict[key] = (current_state_dict[key] + loaded_state_dict[key]) / 2 
