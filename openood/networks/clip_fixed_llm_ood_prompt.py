...
#from tools.classname import inaturalist_top1000_llm_label_list, sun_top1000_llm_label_list, places_top1000_llm_label_list, dtd_top1000_llm_label_list
# from tools.classname import MLLM_ninco_high_freq_pred_llm_simlabel_list, MLLM_ssb_hard_high_freq_pred_llm_simlabel_list
from tools.classname import ninco_high_freq_pred_llm_simlabel_list
from tools.classname import cub_names, food_names, pet_names, car_names
from tools.classname import imagenet_classes, imagenet_near_classnames
import ipdb
import pdb
import json, os
...
imagenet10_classes = ['warplane', 'sportscar', 'bramblingbird', 'Siamesecat',
                       'antelope', 'swissmountaindog', 'bullfrog',
                       'garbagetruck', 'horse', 'containership'
                       ]

imagenet20_classes = ['sailboat', 'canoe', 'balloon', 'tank', 'missile',
                    'bullettrain', 'starfish', 'spottedsalamander', 'commonnewt', 'eft',
                    'zebra', 'greenlizard', 'Africancrocodile', 'Arcticfox', 'timberwolf',
                    'brownbear', 'moped', 'steamlocomotive', 'spaceshuttle', 'snowmobile'
                    ]

def get_class_names(dataset):
    if dataset == 'imagenet' or dataset == 'imagenetv2' or dataset == 'imageneta' or dataset == 'imagenetr' or dataset == 'imagenets':
        return imagenet_classes
        #return imagenet_near_classnames
...
# https://github.com/openai/CLIP/blob/main/notebooks/Prompt_Engineering_for_ImageNet.ipynb
def get_eoe_text_features(model, dataset, text_prompt):
    classnames = eoe_far_llm_label_list
    text_prompt = "simple"
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
...
def get_text_features_offline(model, ens_list):
    classnames = ens_list
    text_prompt = "nice"
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
...
def get_text_features_neg(model, dataset, text_prompt, text_center, ood_number):
    ...
    neglabel_list = selected_adj_text + selected_noun_text
    with torch.no_grad():
        text_features_neg = []
        text_features_dclip = []
        ...
        #text_features_dclip = torch.stack(text_features_dclip, dim=1).cuda() # 512*1000  or  torch.Size([7, 10000, 512])


    text_features_selected = text_features_neg
    # pdb.set_trace()
    return text_features_selected, neglabel_list


class FixedCLIP_LlmOODPrompt(nn.Module):
    def __init__(self, cfg):
        ...
        # pdb.set_trace()
        self.n_output = self.n_cls + cfg.backbone.ood_number 
        ...
        self.text_prompt = cfg.backbone.text_prompt
        self.imagenet_features = get_text_features(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        #self.negative_features = get_text_features_offline(self.model, ninco_high_freq_pred_llm_simlabel_list)
        self.negative_features, self.neglabel_list = get_text_features_neg(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt, cfg.backbone.text_center, cfg.backbone.ood_number)
        self.neglabels_features = self.negative_features
        # self.eoe_list = None
        # self.eoe_text_features = get_eoe_text_features(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.far_nts_list = []
        self.near_nts_list = []
        self.far_nts_features = None
        self.near_nts_features = None
        self.pred = []
        self.high_freq_pred_dict = {}
        self.path_list = []
        self.id_noise_path_list = []
        ...
        self.all_pred_list = []
        self.far_pred_list = []
        self.id_noise_pred_list = []

        self.batch_pred_list = []
        self.ada_threshold = 1
        self.batch_idx = 0
        self.batch_far_labels = []
        self.scores_dict = {}
        #self.batch_image_features = []


        # self.negative_features = get_text_features_ssb_hard(self.model, cfg.backbone.text_prompt)

    def add_far_nts_list(self, batch_far_nts_list):
        self.far_nts_list.extend(batch_far_nts_list)

    def add_near_nts_list(self, batch_near_nts_list): 
        self.near_nts_list.extend(batch_near_nts_list) 

    def add_pred_list(self, batch_pred_list): 
        self.pred.extend(batch_pred_list)         

    def set_near_nts_list(self, near_nts_list):  
        self.near_nts_list = near_nts_list

    def get_negative_text_features(self, neglabels):
        text_prompt = self.text_prompt
        classnames = neglabels # imagenet --> imagenet class names
        ...
        with torch.no_grad():
            ...
            for classname in tqdm(classnames):
                ...
                text_features.append(class_embedding)
            text_features = torch.stack(text_features, dim=1).cuda() # 512*1000
        self.negative_features = text_features

    def get_far_nts_text_features(self, batch_far_labels):
    ...
    def get_near_nts_text_features(self):
        if  len(self.near_nts_list)!=0:
            text_prompt = self.text_prompt
            classnames = self.near_nts_list # imagenet --> imagenet class names
            templates = get_templates(text_prompt)
            with torch.no_grad():
                text_features = []
                for classname in classnames:
                    ...
                    text_features.append(class_embedding)
                text_features = torch.stack(text_features, dim=1).cuda() # 512*1000
            self.near_nts_features = text_features
        else:
            self.near_nts_features = None             

    def reset_negative_text_features(self):
        self.negative_features = self.neglabels_features

    def reset_all_nts_list(self):
        self.far_nts_list = []
        self.near_nts_list = []

    def reset_all_pred_list(self):    
        self.pred = []
        self.high_freq_pred = [] 


    def forward(self, x, return_feat=False):
        self.text_features = torch.cat((self.imagenet_features, self.negative_features), dim=1)
        ...
        if return_feat:
            return image_features, self.text_features.transpose(0,1), logit_scale ## 7*11k*512 --> 11K*7*512
        else:
            logits = logit_scale * image_features @ self.text_features  ## when eval acc, the forward pass is here. 
            return logits