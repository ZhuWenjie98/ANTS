...
from tools.classname_offline import inaturalist_top1000_llm_label_list, places_top1000_llm_label_list, dtd_top1000_llm_label_list
from tools.classname_offline import sun_top1000_llm_label_list, sun_top2000_llm_label_list
from tools.classname_offline import eoe_far_llm_label_list
from tools.classname_gt import sun397_classes, dtd_classes
from tools.classname import food_names, pet_names, dtd_llm_label_gt_list
...
def get_sun_text_features(model, dataset, text_prompt):
...
def get_dtd_text_features(model, dataset, text_prompt):
    text_prompt = "simple"
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
...
def get_ants_text_features(model, dataset, text_prompt):
...
def get_selected_near_ood_text_list(dataset='imagenet', total_ood_num=1000):
    print('the ID dataset is:', dataset)
    print('total_ood_num is:', total_ood_num)
    #######################################
    foot_path = './txtfiles/'
...
def get_text_features_neg(model, dataset, text_prompt, text_center, ood_number):
    ...
    #selected_adj_text, selected_noun_text, unselected_adj_text, unselected_noun_text = get_selected_near_ood_text_list(dataset=dataset, total_ood_num=ood_number)
    ...
    neg_texts = selected_adj_text + selected_noun_text

    #neg_texts = sun_top1000_llm_label_list
    ...
    # pdb.set_trace()
    return text_features_selected, neg_texts
...
class FixedCLIP_NegOODPrompt(nn.Module):
    def __init__(self, cfg):
        ...
        #self.sun_gt_features = get_sun_text_features(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.dtd_gt_features = get_dtd_text_features(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.text_features, self.neg_texts = get_text_features_neg(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt, cfg.backbone.text_center, cfg.backbone.ood_number)
        self.eoe_text_features = get_eoe_text_features(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.ants_text_features = get_ants_text_features(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
    ...
    def forward(self, x, return_feat=False):
        ...
        logit_scale = self.logit_scale.exp()
        #self.text_features = torch.cat((self.imagenet_features, self.eoe_text_features), dim=1)
