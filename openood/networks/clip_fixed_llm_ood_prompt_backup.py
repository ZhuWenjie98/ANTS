...
from tools.classname import sun397_classes, dtd_classes, places_classes, ninco_classes, ssb_hard_class_names
from tools.classname import sun_llm_label_list, places_llm_label_list, dtd_llm_label_list, inaturalist_llm_label_list
from tools.classname import dtd_llm_label_gt_list, inaturalist_llm_label_gt_list, sun_llm_gt_label_list
...
from tools.classname import ssb_hard_top1000_llm_label_list, ninco_top1000_llm_label_list
from tools.classname import ninco_high_freq_pred_llm_simlabel_list, ssb_hard_high_freq_pred_llm_simlabel_list
from tools.classname import MLLM_ssb_hard_high_freq_pred_llm_simlabel_list, MLLM_ninco_high_freq_pred_llm_simlabel_list
import ipdb
import pdb
import json, os
...
def get_text_features_sun(model, dataset, text_prompt):
    classnames = sun_top1000_llm_label_list
    #classnames = sun_llm_gt_label_list
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
        text_features = []
...
def get_text_features_sun_gt(model, dataset, text_prompt):
...
def get_text_features_places(model, dataset, text_prompt):
    #classnames = places_llm_label_list
    classnames = places_top1000_llm_label_list
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
        text_features = []
...
# only for visualization, get the text features of SUN dataset
def get_text_features_dtd(model, dataset, text_prompt):
    classnames = dtd_llm_label_gt_list
    #classnames = dtd_top1000_llm_label_list
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
        text_features = []
...
def get_text_features_dtd_gt(model, dataset, text_prompt):
...
def get_text_features_inaturalist(model, dataset, text_prompt):
    #classnames = inaturalist_llm_label_list
    classnames = inaturalist_top1000_llm_label_list
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
        text_features = []
...
def get_text_features_ssb_hard(model, dataset, text_prompt):
    classnames = MLLM_ssb_hard_high_freq_pred_llm_simlabel_list
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
        text_features = []
...
def get_text_features_ninco(model, dataset, text_prompt):
    classnames = MLLM_ninco_high_freq_pred_llm_simlabel_list
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
        text_features = []
...
def get_text_features_ninco_gt(model, dataset, text_prompt):
    classnames = ninco_classes
    templates = get_templates(text_prompt) # simple --> text prompt for each classes. 
    print('adopt text prompt of', text_prompt)
    with torch.no_grad():
        text_features = []
...
def get_text_features_neg(model, dataset, text_prompt, text_center, ood_number):
    ...
    text_features_selected = torch.cat((text_features, text_features_neg), dim=1)
    # pdb.set_trace()
    return text_features_selected, text_features_unselected


class FixedCLIP_LlmOODPrompt(nn.Module):
    def __init__(self, cfg, neglabels):
        super().__init__()
        backbone = cfg.backbone.name # 'ViT-B/16'
        classnames = get_class_names(cfg.backbone.dataset) # imagenet 
        self.n_cls = len(classnames)
        ...
        self.sun_gt_features = get_text_features_sun_gt(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.places_features = get_text_features_places(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.dtd_features = get_text_features_dtd(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.dtd_gt_features = get_text_features_dtd_gt(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.inaturalist_features = get_text_features_inaturalist(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)

        self.ssb_hard_features = get_text_features_ssb_hard(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.ninco_features = get_text_features_ninco(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)
        self.ninco_gt_features = get_text_features_ninco_gt(self.model, cfg.backbone.dataset, cfg.backbone.text_prompt)

        self.text_features = torch.cat((self.imagenet_features, self.ninco_features), dim=1)
